...
- **Channel search** — `ChannelSearch` (`src/components/ChannelSearch.tsx`) is the container: it owns the search input plus `channelInfo`/`clips`/`playingIndex` state, and composes two presentational children:
  ...
  - `ClipsMenu` (`src/components/ClipsMenu.tsx`) — `Player` for the active clip plus the `ClipTile` grid below it; clicking a tile updates `playingIndex`. Each tile shows the clip's `creatorName` (Helix `creator_name` — the clip's creator, not necessarily the broadcaster) and two overlay icons on the thumbnail (`.clip-tile-actions`) — "↗" to `https://clips.twitch.tv/<slug>`, and "⬇" to download, both calling `stopPropagation()` so they don't also select the tile as active.
  - **Clip downloads are unofficial**: Helix has no documented way to get a clip's video file — only a thumbnail image, embed URL, and page URL. `deriveClipDownloadUrl()` (`src/lib/clipDownload.ts`) derives an `.mp4` URL from the thumbnail URL's naming pattern (`...-preview-WxH.jpg` → `....mp4`), a trick long used by community clip-downloader tools but undocumented — Twitch could change their CDN naming at any time and silently break it. Returns `null` (hiding the download affordance) if a clip has no thumbnail or the pattern doesn't match. Used by both `ClipTile`'s download icon and `Player`'s "⬇ Download" button for the active clip.
